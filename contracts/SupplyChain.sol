...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier paidEnough(uint _price){
    require(msg.value >= _price, "Didnt pay enough");
    _;
  }
  ...
  modifier forSale(uint sku){
    require(items[sku].state == State.ForSale, 'for sale error');
    _;
  }
  modifier sold(uint sku){
    require(items[sku].state == State.Sold, 'sold modifier error');
    _;
  }
  modifier shipped(uint sku) {
    require(items[sku].state == State.Shipped, 'shipped modifier error');
    _;
  }
  modifier received(uint sku){
    require(items[sku].state == State.Received, 'received modifier error');
    _;
  }
